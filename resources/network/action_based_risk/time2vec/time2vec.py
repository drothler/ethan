...
def job(obj, proc_data):
    proc_train_file = open(obj.train_file_name, 'a')
    proc_train_data = obj.get_train_data(proc_data)
    with lock:
...
class Time2Vec(object):
    ...
    def gen_train_data(self):
        ...
        while True:
            ...
            #curr_data['datetime'] = pd.to_datetime(curr_data['datetime'], errors='coerce', format='%Y-%m-%d %H-%M-%S')
            ...
            if not curr_data.empty:
                ...
                if num_processes > 1:
                    ...
                    #print("The most recent data HERE HERE ")
                    #print(curr_data)

                    # Assign data to next available process
                    if len(procs) == num_processes:
                        procs[count] = Process(target=job, args=(self, curr_data))
                    else:
                        procs.append(Process(target=job, args=(self, curr_data)))
                    procs[count].start()
                    count += 1
    ...
    def get_train_data(self, data):
        ...
        columns = ['input', 'target', 'time_difference', 'duration_difference', 'group_size_difference']
        train_data = pd.DataFrame(columns=columns)
        train_data['input'] = train_data['input'].astype(np.int32)
        train_data['target'] = train_data['target'].astype(np.int32)
        train_data['time_difference'] = train_data['time_difference'].astype(np.float32)
        train_data['duration_difference'] = train_data['duration_difference'].astype(np.float32)
        train_data['group_size_difference'] = train_data['group_size_difference'].astype(np.float32)
        ...
        for center_index, center_row in data_df.iterrows():
            ...
            while curr_index < len(data_df.index) and data_df.iloc[curr_index]['id'] == curr_id:
                ...
                time_difference, duration_difference, group_size_difference = self.get_weight(center_row, context_row)
                curr_index += 1
                ...
                forward_row = pd.DataFrame(columns=columns,
                                           data=[[center_row['concept'], context_row['concept'], time_difference, duration_difference, group_size_difference]])
                train_data = train_data.append(forward_row, ignore_index=True)

                # Predict backward
                backward_row = pd.DataFrame(columns=columns,
                                            data=[[context_row['concept'], center_row['concept'], time_difference, duration_difference, group_size_difference]])
    ...
    def get_weight(self, row1, row2):
        ...
        time_difference = abs((datetime.strptime(str(row1["datetime"]), "%Y-%m-%d %H-%M-%S") - datetime.strptime(str(row2["datetime"]), "%Y-%m-%d %H-%M-%S")).total_seconds())
        duration_difference = abs(int(row1["duration"]) - int(row2["duration"]))
        group_size_difference = abs(int(row1["groupSize"]) - int(row2["groupSize"]))

        return time_difference, duration_difference, group_size_difference
    ...
    def learn_embeddings(self):
        ...
        self.load_train_data(self.train_file_name)
        start_time = datetime.now()
	    ...
	    # Whenever you need to record the loss, feed the mean loss to this placeholder
            tf_loss_train = tf.placeholder(tf.float32,shape=None,name='loss_summary_1')
            # Create a scalar summary object for the loss so it can be displayed
            tf_train_loss_summary = tf.summary.scalar('loss1', tf_loss_train)
	    
            # Whenever you need to record the loss, feed the mean loss to this placeholder
            tf_loss_valid = tf.placeholder(tf.float32,shape=None,name='loss_summary_2')
            # Create a scalar summary object for the loss so it can be displayed
            tf_valid_loss_summary = tf.summary.scalar('loss2', tf_loss_valid)
    
        performance_summaries = tf.summary.merge([tf_train_loss_summary,tf_valid_loss_summary])
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.epochs):
                ...
                while True:
                    ...
                    if training_phase:
                        ...
                        if count > self.train_size:
                            ...
                            _,train_loss = sess.run([train, weighted_loss],{inputs: batch_inputs, targets: batch_targets, weights: batch_weights})
                ...
                train_loss_results.append(np.mean(train_loss_per_epoch))
                val_loss_results.append(np.mean(valid_loss_per_epoch))
                print('Average train loss in epoch %d: %.5f'%(i+1,np.mean(train_loss_per_epoch)))    
                print('Average valid loss in epoch %d: %.5f'%(i+1,np.mean(valid_loss_per_epoch)))    
                ...
                if i%2==0:
                    ...
                    plt.legend()
                    plt.savefig(f"./loss_plots/epoch_{i}")

                avg_train_loss = np.mean(train_loss_per_epoch)
                avg_valid_loss = np.mean(valid_loss_per_epoch)
                #summ = sess.run(performance_summaries, feed_dict={tf_loss_train:avg_train_loss, tf_loss_valid:avg_valid_loss})
        ...
        plt.plot(val_loss_results, label = "Validation loss")
        plt.legend()
        plt.savefig(f"./loss_plots/loss_curve")
...
class DataIterator(object):
    ...
    def __init__(self, file_name, chunk_size):
        ...
        """
        self.file = open(file_name)
        columns = ['id', 'datetime', 'concept', 'duration', 'groupSize', 'contact']
