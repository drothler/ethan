...
class NetworkSimulator:
    ...
    def __init__(self, num_nodes, k_degree, p_probability, population_data_path, data_duplicates=False, verbose=False):
        ...
        with open('network/simulation/config.yaml', 'r') as stream:
    ...
    def load_facility_files(self):
        try:
            ...
            if self.data_duplicates:
                ...
                #print(math.floor(len(work) /(118000/self.config['num_nodes'])))
                #print(work[0:math.floor(len(work) /(118000/self.config['num_nodes']))])
            ...
            sys.exit(-1)
        #print(len(work), len(edu))
        
        return work[0:math.floor(len(work) /(118000/self.config['num_nodes']))] + edu[0:math.floor(len(edu) /(118000/self.config['num_nodes']))], homes
